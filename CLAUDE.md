...
2. **Source builds** — `amneziawg-dkms`, `amneziawg-tools`, `mesa-git`, `scx-scheds`, `scx-tools`, `scx-manager`. Real `BuildRequires`, real compilation. The two `amneziawg-*` packages and `mesa-git` track an upstream **git commit** (not a release tag); the three `scx-*` packages track upstream release tags. See versioning below.
